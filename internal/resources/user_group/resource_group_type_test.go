...
func TestAccGroupTypeResource_Basic(t *testing.T) {
	...
	testResourceName := fmt.Sprintf("%s.%s", base.RESOURCE_GROUP_TYPE, testResourceID)
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`Attribute role_mode value must be one of:`),
			},
		},
	})
}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(testResourceName, "group_type", updatedGroupType),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile("The attribute allowed_roles cannot be empty when role_mode is set to"),
			},
		},
	})
}
...
func TestAccGroupTypeResource_UnknownAllowedRolesReference(t *testing.T) {
	...
	testResourceName := fmt.Sprintf("%s.%s", base.RESOURCE_GROUP_TYPE, groupType)
