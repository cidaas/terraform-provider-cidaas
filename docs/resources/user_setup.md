...
Writes (create/update/delete) require admin or developer roles (`USERSETUP_MANAGER` / `APP_MANAGER` / `ADMIN` / `SECONDARY_ADMIN` / `SUPER_ADMIN`) — there is no write OAuth scope. Read-by-ID may use roles or scope `cidaas:usersetup_read`.## Example Usage
