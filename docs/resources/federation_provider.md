---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "cidaas_federation_provider Resource - terraform-provider-cidaas"
subcategory: ""
description: |-
  Manages Native Federated Identity Providers (v4.x) via /federation/providers.
---

# cidaas_federation_provider (Resource)

Manages Native Federated Identity Providers in Cidaas v4.x (Trustdesk) via `/federation/providers`.

~> **Note on Owner Assignment**: All created provider configurations carry `owner: "client"` by default to ensure full visibility and editability in `adminui-srv` and the Admin UI dashboard.

## Example Usage

```terraform
resource "cidaas_federation_provider" "example" {
  provider_name          = "federated_oauth2_provider"
  display_name           = "Federated OAuth2 Provider (v4)"
  standard_type          = "OAUTH2"
  client_id              = "v4-client-id"
  client_secret          = "v4-client-secret"
  authorization_endpoint = "https://auth.example.com/oauth/authorize"
  token_endpoint         = "https://auth.example.com/oauth/token"
  userinfo_endpoint      = "https://auth.example.com/oauth/userinfo"
  logo_url               = "https://cdn.example.com/logo.svg"
  domains                = ["example.com"]
  owner                  = "client"
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `client_id` (String) Client ID of the provider.
- `display_name` (String) Display name of the provider.
- `provider_name` (String) Unique provider name.
- `standard_type` (String) Standard type (`OAUTH2`, `OIDC`, `SAML`, `LDAP`).

### Optional

- `authorization_endpoint` (String) Authorization endpoint URL.
- `client_secret` (String, Sensitive) Client secret of the provider.
- `domains` (List of String) Allowed domains for provider.
- `logo_url` (String) Logo URL of the provider.
- `owner` (String) Owner of the provider (defaults to `client` for Admin UI compatibility).
- `token_endpoint` (String) Token endpoint URL.
- `userinfo_endpoint` (String) Userinfo endpoint URL.

### Read-Only

- `id` (String) Unique identifier of the federation provider.

## Import

Import is supported using the following syntax:

```shell
terraform import cidaas_federation_provider.example "provider_id_123"
```
