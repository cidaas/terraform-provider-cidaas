...
page_title: "cidaas_social_provider Resource - terraform-provider-cidaas"
subcategory: ""
description: |-
...
Manages Social Identity Providers in Cidaas (`v3.x` legacy `/providers-srv/multi/providers` and `v4.x` `/federation/providers`).

~> **Note on Owner Assignment**: For cidaas v4.x (Trustdesk), all created social provider configurations carry `owner: "client"` to guarantee full visibility and editability in the Admin UI dashboard (`adminui-srv`).
...
```shell
terraform import cidaas_social_provider.google "google:provider_id_123"
```
